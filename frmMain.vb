...
Public Class frmMain
    ...
    Public Sub DisplayBin()
        ...
        If aResultPack(1) < lumen_specH And aResultPack(1) > lumen_specL Then     'check lumen
    ...
    Private Sub cbCCT_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cbCCT.SelectedIndexChanged
        ...
        ElseIf Val(BaseFN.Substring(9, 1)) > 4 Then
            bin_spec = 4
        Else
            bin_spec = 3
        End If
    ...
    Public Sub GetCurrentWS(ByVal wsArray As Array, ByVal numB As Integer)  'numB is for making the samples
        ...
        For i = 1 To 3
            ...
            If numB = 1 Then        'making sample set
                If i = 1 Then
                    sampledesc = "Enter Working Sample Set Description and" & vbCrLf & "Install Working Sample #" & i & vbCrLf & "Turn on power supply. Click OK when complete"
                Else
                    sampledesc = "Install Working Sample #" & i & vbCrLf & "Turn on power supply. Click OK when complete"
                End If
            Else                    'reading for verification
                SID = wsID(i - 1)
                sampledesc = "Install Working Std " & wsID(i - 1) & vbCrLf & "Turn on power supply. Click OK when complete"
            End If
            wknum = i
    ...
    Public Sub GetGoldSamples()
        ...
        For i = 0 To gldSamplesCnt - 1
            promptIdx = i
            sampledesc = "Install Gold Sample " & SampleID(i) & vbCrLf & "Turn on power supply. Click OK when complete"
            SID = SampleID(i)
            imsg = DialogTakeSample.ShowDialog()
            If AcqSuccess = False Or imsg = vbCancel Then
                Exit Sub
            End If
            SISData(i, 0) = aResultPack(1)
    ...
    Public Sub ReadGoldFile()
        ...
        Dim gfn As String = dBoxFactory & BaseFN & "\" & BaseFN & ".ref"
    ...
    Private Sub mnuStartJob_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuStartJob.Click
        ...
        'If cbCCT.SelectedText.Contains(id_cct / 100) = False Then         'need to load the correct bin after scan
        'For i = 0 To cbCCT.Items.Count - 1
        'If cbCCT.Items(i).ToString.Contains(id_cct / 100) Then
        ...
        If cbCCT.SelectedText.Equals(BaseFN) = False Then         'need to load the correct bin after scan
            For i = 0 To cbCCT.Items.Count - 1
                If cbCCT.Items(i).ToString.Equals(BaseFN) Then
                    cbCCT.SelectedIndex = i
                    Exit For
                End If
            Next
    ...
    Public Function GetCalSamples() As Boolean
        ...
        For i = 1 To 9
            ...
            If idMsg = vbIgnore Then                    'if user choses to skip disk, assign 0 to offline data.  Sync shall ignore this disk.
                ...
                OffLineData(i - 1, 3) = 0
            Else
                OffLineData(i - 1, 0) = aResultPack(1)  'ce
